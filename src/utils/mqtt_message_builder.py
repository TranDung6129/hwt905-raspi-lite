...
class MQTTMessageBuilder:
    ...
    def create_data_point(self, processed_results: Dict[str, Any], current_timestamp: float) -> Dict[str, Any]:
        ...
        return {
            ...
            "acc_x_filtered": processed_results["acc_x_filtered"][-1] if hasattr(processed_results["acc_x_filtered"], 'size') and processed_results["acc_x_filtered"].size > 0 else (processed_results["acc_x_filtered"] if isinstance(processed_results["acc_x_filtered"], (int, float)) else 0),
            "acc_y_filtered": processed_results["acc_y_filtered"][-1] if hasattr(processed_results["acc_y_filtered"], 'size') and processed_results["acc_y_filtered"].size > 0 else (processed_results["acc_y_filtered"] if isinstance(processed_results["acc_y_filtered"], (int, float)) else 0),
            "acc_z_filtered": processed_results["acc_z_filtered"][-1] if hasattr(processed_results["acc_z_filtered"], 'size') and processed_results["acc_z_filtered"].size > 0 else (processed_results["acc_z_filtered"] if isinstance(processed_results["acc_z_filtered"], (int, float)) else 0),
            "vel_x": processed_results.get("vel_x", 0),
            "vel_y": processed_results.get("vel_y", 0),
            "vel_z": processed_results.get("vel_z", 0),
            "disp_x": processed_results.get("disp_x", 0),
            "disp_y": processed_results.get("disp_y", 0),
            "disp_z": processed_results.get("disp_z", 0),
